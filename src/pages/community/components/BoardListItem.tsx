...
const BoardListItem: React.FC = () => {
  ...
  useEffect(() => {
    getAllCommunityData()
      .then((response) => {
        ...
        setLastPage(Math.ceil(data.data.length / 5));
        setPosts(data.data.slice(0, 5));
        const sortedArr = [...data.data];
        sortedArr.sort((a, b) => b.read_users.length - a.read_users.length);
        console.log(sortedArr); // [3, 1, 2]
      })
      .catch((error) => {
        console.error(error);
      });
  ...
  const onClickPageBtn = (selectedPage: number) => {
    const startIdx = (selectedPage - 1) * 5;
    const endIdx = selectedPage * 5;
