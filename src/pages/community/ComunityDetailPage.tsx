...
import { deleteCommunityByCommunity_no, deleteReply, getDataByCommunity_noAndUser_id } from '../../api/api-community';
...
export const CommunityDetailPage: React.FC = () => {
    ...
    const getDataByCommunity = async (user_id: string) => {
        try {
            const getDataByCommunityResponse = await getDataByCommunity_noAndUser_id(Number(lastPathSegment), user_id);
    ...
    const handleSubmit = async (e: any) => {
        try {
            const postReplyResponse = await postReply(useId, text, Number(lastPathSegment));
    ...
    const handleDelete = async (targetId: number) => {
        try {
            const deleteMyReply = await deleteReply(targetId, String(localStorage.getItem('token')));
            getDataByCommunity(useId);
        } catch (error) {
            console.log(error);
        }
            ...
            <StyledContainer>
                ...
                <StyledCommunityInfo>
                    <StyledCommunityInfoContainer>
                        ...
                        <StyledCommunitySunInfo>{a.updatedAt ? dateSplice(a.updatedAt) : ''}</StyledCommunitySunInfo>
                        <StyledCommunitySunInfo>조회 : {a.read_users.length}</StyledCommunitySunInfo>
                    </StyledCommunityInfoContainer>
                    {/* 현재 나의 id와 글쓴이id가 같으면 수정 / 삭제 버튼이 생김 */}
                    {useId === writerId ? (
                        <FixButtonContainer>
                            <FixButton onClick={() => navigate(`/Community/CommunityEditPage/${lastPathSegment}`)}>
                        ...
                        <div key={b.reply_id} id={b.reply_user_id}>
                            <StyledReplyContainer>
                                ...
                                {b.reply_user_id === useId ? (
                                    <StyledDelButton onClick={() => handleDelete(b.reply_id)}>삭제</StyledDelButton>
                                ) : (
                                    <div></div>
                                )}
                            </StyledReplyContainer>
