...
export const getDataByCommunity_noAndUser_id = async (community_no: number, user_id: string) => {
    const response = await axiosInstance.get(`community/detl?community_id=${community_no}&user_id=${user_id}`);
    return response;
};

/** 4. 커뮤니티 게시글 수정  put */
export const putCommunity = async (community_id: number, title: string, content: string, token: string) => {
...
export const deleteCommunityByCommunity_no = async (community_id: number, token: string) => {
...
/** 6. 커뮤니티 댓글 등록  post */
export const postReply = async (reply_user_id: string, reply_content: string, community_id: number) => {
