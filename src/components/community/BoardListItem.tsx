            ...
            {tap == 1 ? (
                <StyledPostListItemBox>
                    {posts.map((post, index) => (
                        <StyledPostItems onClick={onItemClick} key={index} id={post.community_id}>
                            <StyledLeftPostItem>
                ...
                <StyledPostListItemBox>
                    {posts
                        ...
                        .map((filteredPost, index) => (
                            <StyledPostItems onClick={onItemClick} id={filteredPost.community_id} key={index}>
                                <StyledLeftPostItem>
                                    <StyledPostTitle>{filteredPost.title}</StyledPostTitle>
                                </StyledLeftPostItem>
                                <StyledRightPostItem>
                                    <StyledPostItem>{filteredPost.reply_count}</StyledPostItem>
                                    <StyledPostItem>{filteredPost.read_users.length}</StyledPostItem>
                                    <StyledPostItem>{filteredPost.user_name}</StyledPostItem>
                                    <StyledPostItem>{dateSplice(filteredPost.updatedAt)}</StyledPostItem>
                                </StyledRightPostItem>
                            </StyledPostItems>
                        ))}
                </StyledPostListItemBox>
